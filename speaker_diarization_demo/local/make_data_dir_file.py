...
- wav.scp  --  Kaldi WAV script file with one entry per FLAC file located
  under ``data/wav/``
  if data/sad containing speech regions is provided then 
...
- recording id  --  set to the basename of the corresponding FLAC file minus
  the extension; e.g., the recording id of ``data/wav/DH_DEV_0001.wav`` is
  ``DH_DEV_0001``
...
class Recording:
    ...
    @staticmethod
    def load_recordings(sad_dir, wav_file, rttm_dir=None):
        ...
        audio_dir : str
            Path to directory containing FLAC/WAV files.
...
def main():
    ...
    parser.add_argument(
        'wav_file', metavar='wav-file', type=Path,
        help='path to source FLAC directory')
    parser.add_argument(
        '--sad_dir', metavar='sad-dir', type=Path,
        help='path to source SAD directory',default=None,required=False)
    parser.add_argument(
    ...
    if has_rttm:
        ...
        for turn in turns:
            ...
            if 'laugh' not in turn.speaker_id:
                turn_temp.append(turn)
        turns = turn_temp
        write_rttm_file(
